...
#
#        h.write('')                                                !На переработке. Если шарите за python, можете попробовать
#        m.write('')                                                 Восстановить. Я для этого будильник и не убирал.         !
#
#        h.close()
#        m.close()
...
#-----------------------------Фразы-----------------------------
    ...
    if ok_was:
        if speech.startswith('открой сайт'):
            ...
            tts_speech('Открываю сайт '+speech[12:], 'ru')
            ok_was = False
...
#            ok_was = False
            ...
            webbrowser.open_new(config["Main"]["search"]+speech)
            ...
            time.sleep(0.1)
        elif speech == 'выключи компютер':
            os.system('shutdown /s /c Masha')
            tts_speech('Выключаю', 'ru')
    speech = ''
